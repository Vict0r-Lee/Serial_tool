...
ushort crc16(QByteArray data,unsigned char crc[])
{
    unsigned char crc_hi = 0xFF; /* high CRC byte initialized */
    unsigned char crc_lo = 0xFF; /* low CRC byte initialized */
    int i,j; /* will index into CRC lookup */

    for(i = 0;i < data.count();i++)
    {
        j = crc_hi ^ (unsigned char)data.at(i);
        crc_hi = crc_lo ^ table_crc_hi[j];
        crc_lo = table_crc_lo[j];
    }
    crc[0] = crc_hi;
    crc[1] = crc_lo;
